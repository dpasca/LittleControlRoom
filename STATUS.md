...
- `/resolve` runs conflict repair in a separate managed background engineer lane, preserving any interactive session for the same checkout while projecting live progress and terminal outcomes onto the project row/detail, refreshing Git status after completion, and restoring both lanes independently after a graceful restart.
