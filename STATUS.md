...
- Embedded Codex, OpenCode, Claude Code, and LCAgent panes support live sessions, resume and new flows, model selection, approval and input handling, mixed-provider project workflows, project-scoped runtime and commit commands with in-session dialogs, and graceful-restart journaling that can continue LCR-owned interrupted turns after confirmation. LCR-owned Claude streams disable Claude's native background-task mode so shell commands remain foreground-owned through completion; structured async evidence is still surfaced defensively, with lost ownership reported explicitly.
...
- Codex session cleanup is audit-first: a periodic read-only pass groups old thread trees by retained LCR deleted-worktree evidence, while `/codex-gc` requires explicit selection and a separate permanent-deletion confirmation before using Codex app-server deletion and verifying reclaimed rollout bytes. Background audits never delete sessions.
