...
- The TUI hosts a read-only mobile web surface in-process, using loopback by default with an explicit trusted-LAN listen option. It shares service, store, privacy, and UI-neutral semantic models, overlays nonblocking live engineer snapshots, and falls back to recorded artifact transcripts.
...
- Evolve the mobile surface from read-only monitoring toward authenticated, explicitly confirmed session controls without creating a second domain model.
