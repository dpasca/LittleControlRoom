...
- **Adjust setup** — Use `/settings` for API keys, MLX/Ollama endpoint overrides, paths, refresh timing, and the simplified browser windows control. In `Only when needed`, newly launched embedded Codex sessions now run Playwright through an LCR-managed wrapper with a persistent browser profile, plus a session-local `CODEX_HOME` overlay that shadows only the Playwright skill. When a human step is needed, LCR reveals that same managed browser window instead of handing the URL off to your default browser. Switch to `Classic browser behavior` if you want the original provider-owned behavior, then use `/new-project` for repo-backed work and `/new-task` for quick scratch work.
