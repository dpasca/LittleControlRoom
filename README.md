...
- **Adjust setup** — Use `/settings` for API keys, MLX/Ollama endpoint overrides, paths, refresh timing, and the simplified browser windows control. In `Only when needed`, newly launched embedded Codex and OpenCode sessions now run Playwright through an LCR-managed wrapper with a persistent browser profile. Codex uses a session-local `CODEX_HOME` overlay and OpenCode uses a session-local `XDG_CONFIG_HOME` overlay, both shadowing only the Playwright skill so embedded sessions stay on the managed MCP path without changing your real global installs. When a human step is needed, LCR reveals that same managed browser window instead of handing the URL off to your default browser. Codex currently has the most complete browser-attention UX; OpenCode still trails on richer interaction prompts. Switch to `Classic browser behavior` if you want the original provider-owned behavior, then use `/new-project` for repo-backed work and `/new-task` for quick scratch work.
