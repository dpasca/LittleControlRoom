...
func CuratedTopics() []Topic {
	return cloneTopics([]Topic{
		...
		},
		{
			...
			Kind:    TopicKindWorkflow,
			Surface: SurfaceMainTUI,
			Title:   "Record a Little Control Room demo without restarting",
			Summary: "Start and stop rendered-view recording from the running LCR TUI with /record. LCR saves default recordings under its demo-recordings data directory and keeps the startup recording commands for scripted workflows.",
			Usage: []string{
				"/record",
				"/record start [recording.lcrdemo]",
				"/record stop",
				"/record status",
			...
			ManualSteps: []string{
				"Run /record in the dashboard or an embedded engineer pane to start recording immediately; the REC badge stays visible while capture is active.",
				"Run /record stop to finalize the recording. Bare /record toggles between start and stop, and /record status reports the current path.",
				"Use /record start <recording.lcrdemo> for a custom destination; without a path LCR creates a timestamped recording under its demo-recordings data directory.",
				"For scripted launch-time capture, use lcroom demo record, lcroom tui --demo-record, or make tui-record. Exiting LCR also finalizes any active recording.",
			},
			Related: []string{
				CommandTopicID(SurfaceMainTUI, "record"),
				CommandTopicID(SurfaceMainTUI, "privacy"),
			},
			SourceRefs: []string{"tui.handleDemoRecordingCommand", "demorecord.Controller", "cli.runDemoRecord", "Makefile:tui-record", "docs/reference.md#demo-recordings"},
		},
		{
		...
		{
			ID:      TopicID(SurfaceMainTUI, TopicKindWorkflow, "clean-stale-worktrees"),
			Kind:    TopicKindWorkflow,
			Surface: SurfaceMainTUI,
			...
			SourceRefs: []string{"tui.openStaleWorktreeCleanup", "service.AuditStaleWorktreeCleanup", "service.RevalidateStaleWorktreeCleanupCandidate", "service.FinalizeMergedWorktree", "commands.Specs"},
		},
		{
			ID:      TopicID(SurfaceMainTUI, TopicKindWorkflow, "codex-gc-session-storage"),
			Kind:    TopicKindWorkflow,
			Surface: SurfaceMainTUI,
			Title:   "Garbage-collect old Codex worktree sessions",
			Summary: "Use /codex-gc to audit old Codex trees tied to LCR-deleted worktrees, then explicitly select and permanently delete only fully verified candidates.",
			Usage:   []string{"/codex-gc"},
			ManualSteps: []string{
