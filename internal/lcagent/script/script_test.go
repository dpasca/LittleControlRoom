...
func TestRunnerPhaseWriteGateBlocksLaterPhaseCreateFile(t *testing.T) {
	root := t.TempDir()
	w, err := policy.NewWorkspace(root, policy.AutonomyMedium)
	if err != nil {
		t.Fatal(err)
	...
	defer writer.Close()
	gate := &fakePhaseWriteGate{
		result: PhaseWriteGateResult{
			Allow:                  false,
			FitsActivePhase:        false,
			ContainsLaterPhaseWork: true,
			TooMuchAtOnce:          true,
			Reason:                 "writes HUD and scoring before the render baseline is verified",
			SuggestedSmallerSlice:  "Create only the window, camera, ground, and player marker.",
			Provider:               "fake",
			Model:                  "fake-gate",
		},
	}
	runner := Runner{
		...
		qualityPlan: &QualityPlan{
			...
			Phases: []QualityPlanPhase{
				...
				{Name: "HUD and scoring", Status: "planned", Acceptance: []string{"score text updates"}},
			},
		},
	}
	result, err := runner.RunTool(context.Background(), Action{
		Type: "tool_call",
		Tool: "create_file",
		Args: raw(`{"path":"skate.cpp","content":"// full game with HUD, scoring, environment, tricks\nint main() { return 0; }\n"}`),
	})
	if err == nil || result.Success || !strings.Contains(result.Error, "phase write gate blocked create_file") {
		t.Fatalf("create_file result = %#v err=%v, want phase gate block", result, err)
	}
	if _, err := os.Stat(filepath.Join(root, "skate.cpp")); !os.IsNotExist(err) {
		t.Fatalf("blocked write created file, stat err=%v", err)
	}
	if gate.request.ActivePhase.Name != "render baseline" || gate.request.ActivePhaseIndex != 0 || !strings.Contains(gate.request.ToolArgs, "HUD") {
		t.Fatalf("gate request = %#v", gate.request)
	}
	text := stream.String()
	for _, want := range []string{`"type":"phase_write_gate_result"`, `"allow":false`, `"active_phase":"render baseline"`, `"model":"fake-gate"`} {
		if !strings.Contains(text, want) {
			t.Fatalf("stream missing %s:\n%s", want, text)
		}
	}
...
func TestRunnerPhaseWriteGateAllowsActivePhaseCreateFile(t *testing.T) {
	root := t.TempDir()
	w, err := policy.NewWorkspace(root, policy.AutonomyMedium)
	if err != nil {
		t.Fatal(err)
	...
	gate := &fakePhaseWriteGate{
		result: PhaseWriteGateResult{
			...
			Reason:          "write establishes the render baseline only",
			Provider:        "fake",
			Model:           "fake-gate",
		},
	}
	...
	result, err := runner.RunTool(context.Background(), Action{
		Type: "tool_call",
		Tool: "create_file",
		Args: raw(`{"path":"skate.cpp","content":"// render baseline only\nint main() { return 0; }\n"}`),
	})
	if err != nil || !result.Success {
		t.Fatalf("create_file result = %#v err=%v, want success", result, err)
	}
	if _, err := os.Stat(filepath.Join(root, "skate.cpp")); err != nil {
		t.Fatalf("allowed write did not create file: %v", err)
	}
...
type fakePhaseWriteGate struct {
