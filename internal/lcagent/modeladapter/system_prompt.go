...
func SystemPromptWithOptions(skillIndex, projectInstructions string, opts SystemPromptOptions) string {
	...
	lines = append(lines,
		...
		"For small initial file creation or generated files, prefer create_file with complete content instead of encoding a new file as apply_patch. For phased artifact work, create only the current-phase scaffold or slice first; a requested final single-file artifact should still be assembled through verified phases rather than one giant first write.",
