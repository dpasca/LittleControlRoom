...
func SystemPromptWithOptions(skillIndex, projectInstructions string, opts SystemPromptOptions) string {
	...
	lines = append(lines,
		...
		"When a required quality plan still has planned, in_progress, implemented, or needs_repair phases and there is still useful room to act, keep working the next phase. If you must stop with work remaining, use outcome partial and make the unfinished phases explicit instead of pretending completion.",
