...
func SystemPromptWithOptions(skillIndex, projectInstructions string, opts SystemPromptOptions) string {
	...
	lines = append(lines,
		...
		"When a required quality plan still has planned, in_progress, implemented, or needs_repair phases, keep working the next phase instead of ending with outcome partial. Use blocked or failed only for a concrete stop condition.",
