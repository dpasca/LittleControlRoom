...
func TestRunExecLeadModelOwnsPlanningForSizableWork(t *testing.T) {
	isolateSkillHomes(t)
	root := t.TempDir()
	requests := 0
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if body.Model != "deepseek/test-model" {
			t.Fatalf("model = %q, want lead model", body.Model)
		}
		w.Header().Set("Content-Type", "application/json")
		switch requests {
		case 1:
			for _, msg := range body.Messages {
				if strings.Contains(msg.Content, "make the implementation plan yourself") {
					sawLeadPlanningGuidance = true
				}
			}
						...
						"tool_calls": []any{map[string]any{
							...
							"function": map[string]any{
								...
								"arguments": map[string]any{
									"summary":       "finished by lead model",
									"outcome":       "completed",
									"files_changed": []any{},
									"verification":  []any{},
								},
	...
	if !sawLeadPlanningGuidance {
		t.Fatalf("lead request did not include main-model planning guidance")
	}
	text := stdout.String()
	for _, want := range []string{
		`"type":"turn_complete"`,
...
func TestRunExecCompactsOversizedResumeForSelectedModel(t *testing.T) {
	isolateSkillHomes(t)
	root := t.TempDir()
	dataDir := t.TempDir()
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		for _, msg := range body.Messages {
			...
			if strings.Contains(msg.Content, strings.Repeat("model switch context ", 2000)) {
				t.Fatalf("compacted request kept raw oversized prior answer")
			}
		}
		if !currentStandalone || staleStandalone || !compactedContextSeen {
			t.Fatalf("compacted request current=%v stale=%v compacted=%v messages=%#v", currentStandalone, staleStandalone, compactedContextSeen, body.Messages)
		}
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{
			"id":"resp_final",
