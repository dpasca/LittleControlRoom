...
func TestLowAutonomyVerificationCommandPolicy(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "pnpm version probe", argv: []string{"pnpm", "--version"}, allow: true},
		{name: "npm version probe", argv: []string{"npm", "--version"}, allow: true},
		{name: "node version probe", argv: []string{"node", "--version"}, allow: true},
		{name: "corepack version probe", argv: []string{"corepack", "--version"}, allow: true},
		{name: "package manager version subcommand", argv: []string{"yarn", "version"}, allow: false},
		{name: "corepack enable mutates shims", argv: []string{"corepack", "enable"}, allow: false},
		{name: "which package manager", argv: []string{"which", "pnpm"}, allow: true},
		{name: "which all package managers", argv: []string{"which", "-a", "node", "pnpm"}, allow: true},
		{name: "which rejects path probes", argv: []string{"which", "../pnpm"}, allow: false},
