...
func Parse(subcmd string, args []string) (AppConfig, error) {
	...
	lcagentCriticProvider := fs.String("lcagent-critic-provider", cfg.LCAgentCriticProvider, "LCAgent final-review critic provider: off, main, openrouter, openai, deepseek, moonshot, or xiaomi")
	lcagentCriticModel := fs.String("lcagent-critic-model", cfg.LCAgentCriticModel, "LCAgent final-review critic model; blank with provider main uses the main model")
	lcagentCriticReasoning := fs.String("lcagent-critic-reasoning-effort", cfg.LCAgentCriticReasoning, "optional LCAgent final-review critic reasoning effort, for example low")
