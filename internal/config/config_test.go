...
func TestParseLoadsEditableSettingsFromConfigFile(t *testing.T) {
	...
	content := "" +
		...
		"boss_chat_ollama_thinking = true\n" +
		...
		"lcagent_path = \"~/bin/lcagent\"\n" +
		"lcagent_env_file = \"~/dev/repos/ChatNext3/.env.server.development\"\n" +
		...
		"lcagent_critic_provider = \"main\"\n" +
		"lcagent_critic_model = \"gpt-5.5\"\n" +
		"lcagent_critic_reasoning_effort = \"high\"\n" +
	...
	if !cfg.BossChatOllamaThinking {
		t.Fatalf("boss chat ollama thinking = false, want true")
	}
	...
	if got, want := cfg.LCAgentPath, filepath.Join(home, "bin", "lcagent"); got != want {
		t.Fatalf("lcagent path = %q, want %q", got, want)
	}
	if got, want := cfg.LCAgentEnvFile, filepath.Join(home, "dev", "repos", "ChatNext3", ".env.server.development"); got != want {
		t.Fatalf("lcagent env file = %q, want %q", got, want)
	}
	if got, want := cfg.LCAgentRoutePreset, "quality"; got != want {
		t.Fatalf("lcagent route preset = %q, want %q", got, want)
	}
	if got, want := cfg.LCAgentProvider, "deepseek"; got != want {
		t.Fatalf("lcagent provider = %q, want %q", got, want)
	}
	...
	if got, want := cfg.LCAgentContextProfile, "large"; got != want {
		t.Fatalf("lcagent context profile = %q, want %q", got, want)
	}
	if got, want := cfg.LCAgentRequestTimeout, 10*time.Minute; got != want {
	...
	if got, want := cfg.LCAgentCriticProvider, "main"; got != want {
		t.Fatalf("lcagent critic provider = %q, want %q", got, want)
	}
	if got, want := cfg.LCAgentCriticModel, "gpt-5.5"; got != want {
		t.Fatalf("lcagent critic model = %q, want %q", got, want)
	}
	if got, want := cfg.LCAgentCriticReasoning, "high"; got != want {
		t.Fatalf("lcagent critic reasoning = %q, want %q", got, want)
	}
...
func TestParseLoadsEmbeddedModelPreferencesFromConfigFile(t *testing.T) {
	...
	if got, want := cfg.EmbeddedLCAgentModel, "deepseek/deepseek-v4-pro"; got != want {
		t.Fatalf("embedded lcagent model = %q, want %q", got, want)
	}
	if got, want := cfg.EmbeddedLCAgentReasoning, "low"; got != want {
		t.Fatalf("embedded lcagent reasoning = %q, want %q", got, want)
	}
}
...
func TestParseEditableSettings(t *testing.T) {
	...
	if got, want := settings.LCAgentEnvFile, filepath.Join(home, "dev", "repos", "ChatNext3", ".env.server.development"); got != want {
		t.Fatalf("lcagent env file = %q, want %q", got, want)
	}
	if got, want := settings.LCAgentRoutePreset, "quality"; got != want {
		t.Fatalf("lcagent route preset = %q, want %q", got, want)
	}
	if got, want := settings.LCAgentProvider, "deepseek"; got != want {
		t.Fatalf("lcagent provider = %q, want %q", got, want)
	}
	...
	if got, want := settings.LCAgentContextProfile, "large"; got != want {
		t.Fatalf("lcagent context profile = %q, want %q", got, want)
	}
	...
	if got, want := settings.LCAgentCriticProvider, "off"; got != want {
		t.Fatalf("lcagent critic provider = %q, want %q", got, want)
	}
	if got, want := settings.LCAgentCriticModel, ""; got != want {
		t.Fatalf("lcagent critic model = %q, want blank", got)
	}
	if got, want := settings.LCAgentCriticReasoning, ""; got != want {
		t.Fatalf("lcagent critic reasoning = %q, want blank", got)
	}
...
func TestParseNormalizesDirectLCAgentProviderModelPrefixes(t *testing.T) {
	...
	content := "" +
		...
		"lcagent_critic_provider = \"deepseek\"\n" +
		"lcagent_critic_model = \"deepseek/deepseek-v4-pro\"\n"
	if err := os.WriteFile(configPath, []byte(content), 0o644); err != nil {
		t.Fatalf("write config file: %v", err)
	}
	...
	if cfg.LCAgentCriticModel != "deepseek-v4-pro" {
		t.Fatalf("lcagent critic model = %q, want deepseek-v4-pro", cfg.LCAgentCriticModel)
	}
}
...
func TestSaveEditableSettingsWritesReadableTOML(t *testing.T) {
	...
	err := SaveEditableSettings(configPath, EditableSettings{
		...
		EmbeddedLCAgentReasoning:  "low",
		LCAgentPath:               "/usr/local/bin/lcagent",
		LCAgentEnvFile:            "/tmp/openrouter.env",
	...
	if !strings.Contains(text, "embedded_lcagent_reasoning_effort = \"low\"") {
		t.Fatalf("saved config should include embedded lcagent reasoning: %q", text)
	}
	for _, want := range []string{
		"lcagent_path = \"/usr/local/bin/lcagent\"",
		"lcagent_env_file = \"/tmp/openrouter.env\"",
