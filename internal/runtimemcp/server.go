...
func (s *Server) handle(ctx context.Context, req rpcRequest) (rpcResponse, bool) {
	...
	case "initialize":
		...
		instructions := "Little Control Room exposes project runtime tools plus progressively discoverable read and control catalogs. For current LCR state, call list_lcr_queries with one exact domain, then describe_lcr_query and run_lcr_query. Query results are bounded snapshots and never include private-category projects outside the originating project. For skills, plugins, or MCP setup, discover the integrations query/control domains. These queries read native configuration or fetch catalogs; saved configuration is not proof of running-session tool availability. Inspect the provider and user/project scope before proposing changes with the inspected revision. Never submit literal credentials; use environment-variable references and native authentication. Before any engineer or worktree launch, delegation, or model change, discover choices with engineer.models for the explicit provider. Use model or, for Claude Code, resolved_model to reuse a concrete version; native aliases such as opus[1m] are preserved. Display names are not launch IDs. For visible delegated workers, use agent_task.create with model, model_provider and reasoning_effort; agent_task.continue retains the saved choice when these fields are omitted. Inspect requested model_selection separately from observed_model. Coding delegations can set repository_write=true for exclusive managed ownership of the exact affiliated checkout. A clean baseline and idle engineers/processes are required. Inspect task.repository for owner state, changes and blockers; stop worker processes before handoff. Use structured_results=true for typed worker claims and caller review. Discover agent_task.submit_result and agent_task.review_result. Query work.agent_task_get for workflow.run_id; submit a bounded result then end the worker turn. The host waits for idle and releases ownership before notifying the exact live, idle caller. The caller inspects result_revision and records accept or changes_requested after independent verification. These exact-session metadata calls return immediately without another confirmation or operation id; corrections use agent_task.continue, which resumes the worker on the exact reviewed edits rather than a clean checkout. A task created with max_corrections covers that many correction rounds without another confirmation, limited to the original caller, this task, its existing session and its saved model; follow automatic_delivery in the result rather than assuming either way. A free-text ending is unclassified and explicit stop suppresses automatic review. Do not commit by default. Explicit agent_task.close with status=waiting releases an idle worker lease without accepting its result. For actions, use list_control_capabilities, then describe_control_capability before propose_control_operation. When the user asks you to tell, hand off to, continue, trigger, or steer another embedded engineer, inspect the target session and propose engineer.send_prompt instead of asking the operator to relay the message. Supply the matching explicit provider and exact target_session_id for a known Codex, OpenCode, Claude Code, or LCAgent recipient. After confirmation LCR persists the message, steers an eligible active Codex turn, or waits to resume the exact recipient when idle. Every proposed action is validated by LCR. Previously approved project collaboration allows exact-session engineer messages automatically. Follow automatic_delivery and requires_new_user_turn in the result; only stop for confirmation when required. Continue already-authorized work without inventing new approval checkpoints. Other actions still require confirmation. An engineer you launch with todo.create_worktree_and_start_engineer reports its launch turn back to your exact session. Delivered engineer.send_prompt requests also report the completed worker turn to your exact calling session, including existing workers you did not launch; do not poll for completion. Use get_control_operation to inspect delivery, without tight polling or acknowledgment-only reply loops."
...
func controlCatalogTools(structuredTools bool) []mcpTool {
	tools := []mcpTool{
		...
		},
		{
			...
			Description: "Propose one previously described Little Control Room capability. LCR validates and queues the action. Approved project collaboration can deliver exact-session engineer messages automatically. Follow the result: when automatic_delivery is true, continue authorized work without asking; when requires_new_user_turn is true, stop and wait for the operator. Queued is not delivered.",
			InputSchema: map[string]any{
				"type":                 "object",
				"additionalProperties": false,
				"properties": map[string]any{
