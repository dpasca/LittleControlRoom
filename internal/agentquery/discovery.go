...
const (
	...
	FreshnessContract = "Query results are bounded and include an as_of timestamp. Most use persisted snapshots; knowledge queries return built-in documentation, and integrations queries read native configuration or fetch catalogs as their freshness metadata specifies. They do not claim to mirror transient in-memory TUI state or running-session tool availability."
)
