...
func agentTaskRecord(task model.AgentTask) map[string]any {
	...
	return map[string]any{
		...
		"result_lifecycle": map[string]any{
			...
			"ready_at":       formatTime(task.ResultReadyAt),
			"delivered_at":   formatTime(task.ResultDeliveredAt),
			"delivery_error": clip(task.ResultDeliveryError, 800),
			"consumed_at":    formatTime(task.ResultConsumedAt),
			"consumed_by":    task.ResultConsumedBy,
		},
