(() => {
  ...
  function renderProjects() {
    ...
    for (const project of projects) {
      ...
      const button = createElement("button", `project-button bucket-${project.bucket}`);
      button.type = "button";
  ...
  async function openProject(path, updateHistory) {
    ...
    elements.detailState.replaceChildren(createElement("p", "", "Loading project"));
    renderProjects();
    if (updateHistory) {
  ...
  function setConnection(status, label) {
    ...
    if (status !== "live") elements.connectionDot.classList.add(status);
