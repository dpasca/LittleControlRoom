...
func TestHandlerServesMobileAppAndSemanticDashboard(t *testing.T) {
	...
	for _, id := range []string{"attention-count", "active-count", "all-count", "dashboard-live-channels", "transcript-mode", "session-follow-button", "session-composer", "session-message", "session-send-button"} {
