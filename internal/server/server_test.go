...
func TestHandlerServesMobileAppAndSemanticDashboard(t *testing.T) {
	...
	for _, id := range []string{"attention-count", "active-count", "all-count", "dashboard-live-channels", "command-button", "session-action-deck", "session-project-summary", "quick-panel-dialog", "transcript-mode", "session-follow-button", "session-composer", "session-message", "session-send-button"} {
