...
func bossControlReferenceSpec(domain string) ([]control.CapabilityName, []string) {
	...
	case bossPlannerDomainProjectWork:
		return []control.CapabilityName{
				...
				"For engineer and agent-task model choices, discover exact IDs and efforts with the engineer.models query. Task continuation retains its saved choice when model fields are omitted. Set select_model=true to open the operator picker before launch; reveal only shows the session.",
