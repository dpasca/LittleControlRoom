...
func (m Model) openBossOpenTargetPicker() (tea.Model, tea.Cmd) {
	...
	if len(targets) == 0 {
		m.status = "No files or links in this Chat"
		return m, nil
	}
...
func (m Model) updateBossOpenTargetPicker(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	if len(picker.Targets) == 0 {
		m.closeBossOpenTargetPicker("No files or links in this Chat")
		return m, nil
	}
...
func (m Model) renderBossOpenTargetPickerContent(width, bodyH int) string {
	...
	lines := []string{
		bossMutedStyle.Render(fitLine("Files and links found in this Chat. Enter opens with the system app or browser.", width)),
