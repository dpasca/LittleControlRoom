...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case AssistantReplyMsg:
		return m.applyAssistantReply(msg.response, msg.err)
	...
	case assistantStreamMsg:
		...
		if msg.envelope.done {
			return m.applyAssistantReply(msg.envelope.response, msg.envelope.err)
		}
...
func (m Model) applyAssistantReply(response AssistantResponse, err error) (tea.Model, tea.Cmd) {
	m.sending = false
...
func (m Model) askAssistantStreamCmd(streamID int, messages []ChatMessage, snapshot StateSnapshot, view ViewContext) tea.Cmd {
	...
	return func() tea.Msg {
		...
		go func() {
			...
			select {
			case events <- assistantStreamEnvelope{response: resp, err: err, done: true}:
			case <-ctx.Done():
				select {
				case events <- assistantStreamEnvelope{err: ctx.Err(), done: true}:
				default:
				}
			}
		}()
