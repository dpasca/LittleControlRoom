...
func bossActionPlannerSystemPrompt() string {
	return strings.Join([]string{
		...
		"Available control action kind: propose_control with control_capability equal to engineer.send_prompt, agent_task.create, agent_task.continue, agent_task.close, scratch_task.archive, or todo.add.",
		...
		"For propose_control, the prompt field is the boss-reframed executable task for the engineer session or task. For todo.add, leave prompt empty and put the durable backlog item in todo_text.",
	...
	return map[string]any{
		...
		"properties": map[string]any{
			...
			"control_capability": map[string]any{
				"type":        "string",
				"enum":        []string{"", string(control.CapabilityEngineerSendPrompt), string(control.CapabilityAgentTaskCreate), string(control.CapabilityAgentTaskContinue), string(control.CapabilityAgentTaskClose), string(control.CapabilityScratchTaskArchive), string(control.CapabilityTodoAdd)},
			...
			"todo_text": map[string]any{
				"type":        "string",
				"description": "For todo.add proposals, the durable project TODO text to add. Otherwise empty.",
			},
			...
			"goal_allowed_capabilities": map[string]any{
				"type":        "array",
				"items":       map[string]any{"type": "string", "enum": []string{string(control.CapabilityEngineerSendPrompt), string(control.CapabilityAgentTaskCreate), string(control.CapabilityAgentTaskContinue), string(control.CapabilityAgentTaskClose), string(control.CapabilityScratchTaskArchive), string(control.CapabilityTodoAdd)}},
			...
			"goal_plan_steps": map[string]any{
				...
				"items": map[string]any{
					...
					"properties": map[string]any{
						...
						"capability": map[string]any{"type": "string", "enum": []string{"", string(control.CapabilityEngineerSendPrompt), string(control.CapabilityAgentTaskCreate), string(control.CapabilityAgentTaskContinue), string(control.CapabilityAgentTaskClose), string(control.CapabilityScratchTaskArchive), string(control.CapabilityTodoAdd)}},
