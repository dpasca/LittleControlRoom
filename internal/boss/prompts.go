...
var bossSharedEngineerOutputPrompt = []string{
	...
	"When a delegated agent task is waiting for review, summarize the result and recommend one next move: close it, keep it open, or continue the task with a sharper question.",
}
...
var bossScopedPlannerContextPrompt = []string{
	...
	"Keep an acronym, alias, codename, or partial project name unchanged when passing it to search_context. Never expand it into a similar loaded project name or path without query evidence.",
	...
	"When the user names an exact numeric project TODO ID, use todo_report with that number in todo_id; no project target is required.",
	...
	"Use project_scout when repository files, plans, docs, configuration, source, or tests are needed as fresh evidence; ground its exact target through search_context or another current project query first, and do not infer that file-backed work is absent without that inspection.",
	"A request for news, progress, or current status calls for project/task state first, not Repository Scout. Repository Scout reads one checkout; it is not web research and is not a substitute for current external-event news.",
...
var bossPlannerTaskIdentityPrompt = []string{
	"For delegated work, treat the linked work session as part of that same task unless the data says it changed; do not imply a fresh unrelated session when continuing a task.",
}
...
var bossPlannerReadOnlyPrompt = []string{
	...
	"Use goal_run_report when the user asks what LCR goal runs have happened, whether a multi-step goal was completed, what was verified, what failed, or asks to inspect goal-run traces.",
}
...
var helpPlannerReadOnlyPrompt = []string{
	...
	"Use todo_report for project TODO questions. When the user names an exact numeric TODO ID, put that number in todo_id; no project target is required.",
...
var bossPlannerProposalPayloadPrompt = []string{
	...
	"For agent_task.create, task_kind must be agent unless parent_task_id is set and the user asked for a subagent; put the originating project/TODO when known plus affected PIDs, ports, files, sessions, or related tasks in resources; put allowed action namespaces such as process.inspect, process.terminate, repo.edit, test.run, browser.inspect in capabilities.",
...
var bossPlannerContextLookupPrompt = []string{
	...
	"Keep the user's acronym, alias, codename, or partial project name unchanged in search_context. Never expand it into a similar loaded project name or path without query evidence.",
	...
	"Use project_scout when the answer depends on fresh repository files: plans, roadmaps, documentation, source code, configuration, tests, or whether a file-backed feature exists. Ground its exact target through search_context or another current project query first, and put the user's concrete repository question in query.",
	"A request for news, progress, or current status calls for project/task state first, not Repository Scout. Repository Scout reads one checkout; it is not web research and is not a substitute for current external-event news.",
...
func bossReadOnlyRouterSystemPromptForRequest(req AssistantRequest) string {
	if req.HelpChat {
		return strings.Join([]string{
			...
			"Use todo_report for project TODO questions. When an exact numeric TODO ID is named, put that number in todo_id; no project target is required.",
			"Repository Scout is intentionally unavailable to this fast router because the main planner must ground its repository target first. For fresh repository-file questions, choose kind=pass with planner_domain=inspection.",
			"For an acronym, alias, codename, or partial project name, use search_context with the user's term unchanged when one lookup is sufficient; otherwise choose pass. Never expand it into a similar loaded project name or path.",
			"A request for news, progress, or current status is not by itself a repository-file inspection. Prefer project_detail or search_context for LCR state, and choose pass for current external or web news.",
