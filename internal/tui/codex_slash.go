...
func (m Model) renderCodexSlashBlocks(width int) []string {
	...
	suggestions := m.codexSlashSuggestions()
	if len(suggestions) == 0 {
		lines = append(lines, commandPaletteHintStyle.Render("No supported embedded or host slash commands match. Try /new, /sessions, /model, /terminal, /skills, or /reconnect."))
	} else {
