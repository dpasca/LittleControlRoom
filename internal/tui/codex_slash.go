...
func codexHostSlashCommand(input string) (commands.Invocation, bool) {
	...
	switch inv.Kind {
	case commands.KindTaskActions, commands.KindResolve:
		return inv, true
	default:
		return commands.Invocation{}, false
	}
...
func (m Model) renderCodexSlashBlocks(width int) []string {
	...
	suggestions := m.codexSlashSuggestions()
	if len(suggestions) == 0 {
		lines = append(lines, commandPaletteHintStyle.Render("No supported embedded or host slash commands match. Try /new, /sessions, /model, /critic, /terminal, /skills, or /reconnect."))
	} else {
