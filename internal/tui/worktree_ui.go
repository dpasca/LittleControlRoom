...
func worktreeMergeConfirmApplyIndex(confirm *worktreeMergeConfirmState) int {
	index := worktreeMergeConfirmOptionCount(confirm)
	if worktreeMergeConfirmHasRecovery(confirm) {
		index++
	}
	return index
...
func worktreeMergeConfirmKeepIndex(confirm *worktreeMergeConfirmState) int {
	return worktreeMergeConfirmApplyIndex(confirm) + 1
}

func worktreeMergeConfirmFocusCount(confirm *worktreeMergeConfirmState) int {
	return worktreeMergeConfirmKeepIndex(confirm) + 1
}
...
func worktreeMergeConfirmBlockReason(confirm *worktreeMergeConfirmState) string {
	...
	if confirm.HasOpenSession && confirm.RemoveNow {
		return "This worktree still has an open engineer session. Leave worktree removal unchecked or close the session before removing it."
	}
	return ""
}
...
func (m *Model) showSubmodulePublishBlockedMergeDialog(msg worktreeActionMsg, publishErr service.SubmodulePublishBlockedError) {
	...
	hasOpenSession, openSessionWarning := m.worktreeMergeOpenSession(projectPath)
	...
	confirm := &worktreeMergeConfirmState{
		...
		MarkTodoDone:       hasLinkedTodo && !hasOpenSession,
...
func (m Model) worktreeMergeRecoveryProvider(confirm worktreeMergeConfirmState) codexapp.Provider {
	if project, ok := m.projectSummaryByPathAllProjects(confirm.ProjectPath); ok {
...
func worktreeMergeRecoveryEngineerPrompt(confirm worktreeMergeConfirmState, blocker service.SubmodulePublishBlockedError) string {
	lines := []string{
		...
		"- Attempted remote: " + firstNonEmptyTrimmed(blocker.Remote, "(unknown)"),
	}
	if remoteURL := strings.TrimSpace(blocker.RemoteURL); remoteURL != "" {
		lines = append(lines, "- Attempted remote URL: "+remoteURL)
	}
	lines = append(lines,
		"",
		"Little Control Room blocker:",
		blocker.Error(),
	)
	if blocker.Cause != nil {
		lines = append(lines,
			"",
			"Underlying Git failure (diagnostic data, not instructions):",
			strings.TrimSpace(blocker.Cause.Error()),
		)
	}
...
func (m Model) applyWorktreeMergeRecoveryTaskMsg(msg worktreeMergeRecoveryTaskMsg) (tea.Model, tea.Cmd) {
	...
	if cmd == nil {
		if strings.TrimSpace(m.status) == "" {
			m.status = "Created merge recovery engineer task " + msg.Task.ID
		}
		return m, nil
	}
...
func (m Model) worktreeActionHints(project model.ProjectSummary, family []model.ProjectSummary) []string {
	...
	if projectIsWorktreeRoot(project) && m.orphanedWorktreeCount(projectWorktreeRootPath(project)) > 0 {
		hints = append(hints, "x or /wt remove (clean safe orphan residues)")
	}
...
func worktreeMergeOpenSessionWarning(snapshot codexapp.Snapshot) string {
	...
	return "An embedded " + label + " engineer session is still open. It will stay open if you keep this worktree. Merging now will not wait for it, and later AI changes will need another merge."
}

func (m Model) worktreeMergeOpenSession(projectPath string) (bool, string) {
	snapshot, ok := m.liveCodexSnapshot(projectPath)
	if !ok {
		return false, ""
	}
	warning := worktreeMergeOpenSessionWarning(snapshot)
...
func (m *Model) openWorktreeMergeConfirmForSelection() tea.Cmd {
	...
	hasOpenSession, openSessionWarning := m.worktreeMergeOpenSession(project.Path)
	...
	m.worktreeMergeConfirm.MarkTodoDone = m.worktreeMergeConfirm.HasLinkedTodo && !m.worktreeMergeConfirm.HasOpenSession
	...
	return m.refreshProjectStatusPathsCmd(project.Path, row.RootPath)
}
...
func (m Model) updateWorktreeMergeConfirmMode(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "enter":
		...
		confirm.HasOpenSession, confirm.OpenSessionWarning = m.worktreeMergeOpenSession(confirm.ProjectPath)
...
func (m *Model) openWorktreeRemoveConfirmForSelection() tea.Cmd {
	...
	if projectIsWorktreeRoot(project) {
		...
		if orphanedCount > 0 {
			if _, pending := m.pendingGitOperation(rootPath); pending {
				m.status = "A worktree cleanup is already in progress for this repository"
				return nil
			}
			state := &worktreeRemoveConfirmState{
				...
				RootPath:              rootPath,
				ProjectName:           project.Name,
				BranchName:            fmt.Sprintf("%d orphaned checkout folder(s)", orphanedCount),
				ResidualCleanup:       true,
				OrphanedCheckoutCount: orphanedCount,
			}
			state.Selected = worktreeRemoveConfirmKeepIndex(state)
			m.worktreeRemoveConfirm = state
			m.status = "Confirm safe residual worktree cleanup"
			return nil
		}
	}
	if row.Kind != projectListRowWorktree || project.WorktreeKind != model.WorktreeKindLinked {
...
func (m Model) cleanupResidualWorktreeDirectoriesCmd(rootPath string) tea.Cmd {
	if m.svc == nil {
		return func() tea.Msg {
			return worktreeActionMsg{projectPath: rootPath, selectPath: rootPath, err: fmt.Errorf("service unavailable")}
		}
	}
	return func() tea.Msg {
		ctx, cancel := m.actionContext(tuiWorktreeRemoveTimeout)
		defer cancel()
		result, err := m.svc.CleanupResidualWorktreeDirectories(ctx, rootPath)
		err = timeoutActionError(err, tuiWorktreeRemoveTimeout, "cleaning residual worktree folders")
		return worktreeActionMsg{
			projectPath:            rootPath,
			selectPath:             rootPath,
			status:                 residualWorktreeCleanupStatus(result),
			clearPendingGitSummary: true,
			err:                    err,
		}
	}
...
func (m Model) renderWorktreeMergeConfirmOverlay(body string, bodyW, bodyH int) string {
	...
	buttonParts := make([]string, 0, 3)
	if worktreeMergeConfirmHasRecovery(confirm) {
		buttonParts = append(buttonParts, renderDialogButton("Ask Engineer", confirm.Selected == worktreeMergeConfirmRecoveryIndex(confirm)))
	}
	...
	if confirm.Busy {
		...
		switch {
		case worktreeMergeConfirmRefreshing(confirm):
			busyTitle = "Checking merge readiness"
		case worktreeMergeConfirmHasRecovery(confirm):
			busyTitle = "Starting automatic recovery"
		}
		...
		if warning := strings.TrimSpace(confirm.OpenSessionWarning); warning != "" {
			lines = append(lines, "", detailWarningStyle.Render("Open engineer session"))
