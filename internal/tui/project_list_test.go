...
func TestCodexFooterStatusDoesNotCountIdleExternalSessionAsRunning(t *testing.T) {
	now := time.Date(2026, 7, 27, 8, 36, 0, 0, time.UTC)
	snapshot := codexapp.Snapshot{
		Started:      true,
		Busy:         false,
		BusyExternal: true,
	...
	if got := codexFooterStatus(snapshot, now); got != snapshot.Status {
		t.Fatalf("codexFooterStatus() = %q, want idle external status %q", got, snapshot.Status)
	}
	if codexSnapshotCanSubmitBusyInput(snapshot) {
		t.Fatal("idle external session should remain read-only")
	}
	if got := pickerSummaryForLiveSnapshot(snapshot); got != "Open elsewhere: embedded view is read-only" {
		t.Fatalf("pickerSummaryForLiveSnapshot() = %q, want idle external ownership summary", got)
	}
}
