...
func TestSettingsPrivacyPatternsRevealActionIsDiscoverable(t *testing.T) {
	...
	_ = m.setSettingsSection(settingsSectionIndexByID(settingsSectionScope))
	_ = m.setSettingsSelection(settingsFieldPrivacyPatterns)

	rendered := ansi.Strip(m.renderSettingsContent(100, 24))
	if strings.Contains(rendered, "super-private-folder") || strings.Contains(rendered, "customer-999") {
		t.Fatalf("privacy patterns should start hidden: %q", rendered)
	}
	if !strings.Contains(rendered, "Enter") || !strings.Contains(rendered, "edit") {
		t.Fatalf("privacy patterns row should advertise edit action: %q", rendered)
	}
	if !strings.Contains(rendered, "ctrl+r") || !strings.Contains(rendered, "reveal") {
		t.Fatalf("privacy patterns row should advertise reveal action separately: %q", rendered)
	}

	updated, cmd := m.updateSettingsMode(tea.KeyMsg{Type: tea.KeyCtrlR})
	if cmd != nil {
		t.Fatalf("privacy reveal should not return a command")
	}
	got := updated.(Model)
	if !got.settingsRevealPrivacy || got.status != "Privacy patterns revealed" {
		t.Fatalf("privacy reveal state/status = %v/%q, want revealed", got.settingsRevealPrivacy, got.status)
	}
	rendered = ansi.Strip(got.renderSettingsContent(100, 24))
	if !strings.Contains(rendered, "super-private-folder") || !strings.Contains(rendered, "customer-999") {
		t.Fatalf("revealed privacy patterns should show the editable value: %q", rendered)
	}
	if !strings.Contains(rendered, "ctrl+r") || !strings.Contains(rendered, "hide") {
		t.Fatalf("revealed privacy patterns row should advertise hide action: %q", rendered)
	}

	updated, cmd = got.updateSettingsMode(tea.KeyMsg{Type: tea.KeyCtrlR})
	if cmd != nil {
		t.Fatalf("privacy hide should not return a command")
	}
	got = updated.(Model)
	if got.settingsRevealPrivacy || got.status != "Privacy patterns hidden" {
		t.Fatalf("privacy reveal state/status = %v/%q, want hidden", got.settingsRevealPrivacy, got.status)
	}
}

func TestSettingsPrivacyPatternsEditorSavesWrappedList(t *testing.T) {
	settings := config.EditableSettingsFromAppConfig(config.Default())
	settings.PrivacyPatterns = []string{"super-private-folder", "customer-999"}
	...
	updated, cmd := m.updateSettingsMode(tea.KeyMsg{Type: tea.KeyEnter})
	got := updated.(Model)
	if got.settingsPrivacyEditor == nil {
		t.Fatalf("Enter on privacy patterns should open the wrapped editor")
	}
	if cmd == nil {
		t.Fatalf("opening privacy editor should focus the textarea")
	}
	rendered := ansi.Strip(got.renderSettingsPrivacyEditorContent(80, 24))
	if !strings.Contains(rendered, "super-private-folder") || !strings.Contains(rendered, "customer-999") {
		t.Fatalf("privacy editor should show the existing values: %q", rendered)
	}
	if !strings.Contains(rendered, "ctrl+s") || !strings.Contains(rendered, "save") || !strings.Contains(rendered, "Enter") || !strings.Contains(rendered, "newline") {
		t.Fatalf("privacy editor should show editor actions: %q", rendered)
	}

	got.settingsPrivacyEditor.Input.SetValue("alpha\nbeta, gamma\n")
	updated, cmd = got.updateSettingsPrivacyEditorMode(tea.KeyMsg{Type: tea.KeyCtrlS})
	got = updated.(Model)
	if got.settingsPrivacyEditor != nil {
		t.Fatalf("saving privacy editor should close it")
	}
	if got.settingsFieldValue(settingsFieldPrivacyPatterns) != "alpha,beta,gamma" {
		t.Fatalf("privacy field value = %q, want normalized comma list", got.settingsFieldValue(settingsFieldPrivacyPatterns))
	}
	if !got.settingsSaving {
		t.Fatalf("saving privacy editor should submit settings")
	}
	if cmd == nil {
		t.Fatalf("saving privacy editor should return the settings save command")
	}
}

func TestSettingsPrivacyPatternsEditorEscCancels(t *testing.T) {
	settings := config.EditableSettingsFromAppConfig(config.Default())
	settings.PrivacyPatterns = []string{"super-private-folder", "customer-999"}
	...
	got := updated.(Model)
	got.settingsPrivacyEditor.Input.SetValue("changed")
	updated, cmd := got.updateSettingsPrivacyEditorMode(tea.KeyMsg{Type: tea.KeyEsc})
	got = updated.(Model)
	if got.settingsPrivacyEditor != nil {
		t.Fatalf("Esc should close the privacy editor")
	}
	if got.settingsFieldValue(settingsFieldPrivacyPatterns) != "super-private-folder,customer-999" {
		t.Fatalf("Esc should leave privacy value unchanged, got %q", got.settingsFieldValue(settingsFieldPrivacyPatterns))
	}
	if got.status != "Privacy patterns edit canceled" {
		t.Fatalf("cancel status = %q", got.status)
	}
	if cmd == nil {
		t.Fatalf("closing editor should refocus the privacy row")
	}
}
