...
func TestBuildProjectLCAgentTraceQualityAggregatesRecentSessions(t *testing.T) {
	...
	latest := writeTUILCAgentQualityFixture(t, dir, "lca_latest", "/tmp/demo", strings.Join([]string{
		...
		`{"type":"turn_complete","session_id":"lca_latest","summary":"fixed tests","final_outcome":"completed","files_changed":["main.go"],"verification_status":"verified","actual_checks":[{"command":"go test ./...","status":"passed","success":true}]}`,
	}, "\n")+"\n")
	...
	for _, want := range []string{"latest trace quality:", "outcome completed", "checks: go test ./...", "last 2 sessions", "verified 1/2", "provider failures 1", "provider retries 1", "pending 1", "continuations 1"} {
...
func TestRenderDetailContentShowsLCAgentTraceQuality(t *testing.T) {
	...
	for _, want := range []string{"LCAgent trace", "latest trace quality: 91/good", "outcome partial", "checks: go test ./... passed", "provider retries 1"} {
