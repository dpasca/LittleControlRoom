...
func TestEngineerSendPromptCapabilityMetadata(t *testing.T) {
	...
	for _, provider := range []Provider{ProviderCodex, ProviderOpenCode, ProviderClaudeCode, ProviderLCAgent} {
		if !stringSliceContains(providers[provider].Features, FeatureTargetSession) ||
			!stringSliceContains(providers[provider].Features, FeatureDurableQueue) {
			t.Fatalf("%s features = %#v, want exact-session targeting and durable queueing", provider, providers[provider].Features)
		}
	}
	...
	outputProperties, _ := capability.OutputSchema["properties"].(map[string]any)
	delivery, _ := outputProperties["delivery"].(map[string]any)
	deliveryProperties, _ := delivery["properties"].(map[string]any)
	if _, ok := deliveryProperties["message_id"]; !ok {
		t.Fatalf("engineer.send_prompt output schema = %#v, want durable delivery receipt", capability.OutputSchema)
	}
