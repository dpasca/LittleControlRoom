...
func (s *Store) UpdateAgentTask(ctx context.Context, input model.UpdateAgentTaskInput) (model.AgentTask, error) {
	...
	if input.ObservedModel != nil {
		set = append(set, "observed_model_json = ?")
		args = append(args, encodeAgentTaskModel(*input.ObservedModel))
	}
	if input.Title != nil {
...
func scanAgentTask(scanner interface {
	...
	if err := json.Unmarshal([]byte(observedJSON), &task.ObservedModel); err != nil {
		return model.AgentTask{}, fmt.Errorf("decode task reported model: %w", err)
	}
...
func encodeAgentTaskModel(selection model.AgentTaskModelSelection) string {
	data, _ := json.Marshal(selection)
	return string(data)
}

// RecordAgentTaskObservedModel cannot let a replaced session overwrite the
// currently attached worker's reported model.
func (s *Store) RecordAgentTaskObservedModel(ctx context.Context, taskID, sessionID string, choice model.AgentTaskModelSelection) error {
	if strings.TrimSpace(sessionID) == "" || strings.TrimSpace(choice.Model) == "" {
		return nil
	}
	encoded := encodeAgentTaskModel(choice)
	_, err := s.db.ExecContext(ctx, `UPDATE agent_tasks SET observed_model_json = ?
 WHERE id = ? AND provider = ? AND session_id = ? AND observed_model_json != ?`, encoded, taskID, string(choice.Provider), sessionID, encoded)
	return err
}
