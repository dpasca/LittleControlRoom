...
For LAN mobile access, use the Mobile card in `/setup` or the Mobile section in `/settings`, choose `Phones on this LAN`, and restart LCR. This friendly mode derives the technical `0.0.0.0:<port>` listener; `This computer only` derives `127.0.0.1:<port>`, while `Custom address` preserves direct `host:port` control. The top-right `/mobile` badge adds `LAN`, `SETUP`, `OFF`, or `ERR` when space permits. `/mobile` opens a read-only status panel with the active listener, detected private IPv4 addresses, phone-ready URL, pairing code, and saved next-launch setup. Press Enter to open the existing Mobile setup drilldown or `c` to copy a reachable phone URL. A successful pairing stores a 30-day HTTP-only browser cookie signed by `mobile-auth.key` beside the active database. Loopback listeners do not require pairing. Pairing does not add TLS, so direct HTTP exposure should remain on a trusted LAN.
