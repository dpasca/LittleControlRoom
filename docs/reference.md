...
- `/codex-gc`: Leave the embedded pane visible behind the global Codex session-storage cleanup dialog. `C` switches between Orphaned worktrees (default) and Stale sessions in existing folders (inactive at least seven days; newest root session, pinned, LCR-loaded, and uncertain trees retained). Stale rows summarize REMOVE / KEEP counts per folder and require the same separate permanent-deletion confirmation. `V` or Tab switches to a read-only, largest-first view of storage retained outside the current category.
