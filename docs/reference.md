...
- While boss chat is visible, local slash commands include `/new [prompt]`, `/sessions [session-id]`, `/session [session-id]`, `/resume [session-id]`, `/help`, and `/boss off`. Boss slash commands use the same Tab and Shift+Tab autocomplete behavior as embedded slash commands.
