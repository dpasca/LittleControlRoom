...
For LAN mobile access, use the Mobile card in `/setup` or the Mobile section in `/settings` to save a non-loopback address such as `0.0.0.0:7777`, then restart LCR. The top-right `M:LOCAL`, `M:LAN`, `M:OFF`, or `M:ERR` badge advertises the current state. `/mobile` opens a read-only status panel with the active listener, detected private IPv4 addresses, phone-ready URL, pairing code, and saved next-launch setup. Press Enter to open the existing Mobile setup drilldown or `c` to copy a reachable phone URL. A successful pairing stores a 30-day HTTP-only browser cookie signed by `mobile-auth.key` beside the active database. Loopback listeners do not require pairing. Pairing does not add TLS, so direct HTTP exposure should remain on a trusted LAN.
