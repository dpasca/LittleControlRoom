...
- `/handoff [note]` is the fresh-thread recovery path when the current stream or session cannot be trusted to answer. LCR writes the continuation brief itself from cached structured entries, then opens a fresh session only after that write succeeds; if provider launch subsequently fails, the error retains the brief path for manual recovery.
- `/lcagent-handoff [note]` is the cross-provider recovery path. It does not submit a compaction request to the blocked source provider; LCR writes the brief first and then starts LCAgent with only the brief's private local path in the launch prompt.
