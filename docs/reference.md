...
- `/remove` asks for confirmation, then makes the selected item go away using the safest matching action: it opens scratch-task archive/delete actions, cleans up linked worktrees, removes missing folders from the dashboard, or hides a regular project's exact path from the list. A linked worktree with an originating TODO offers to mark that TODO done before removal even when the branch has not been merged; clean checkouts default to completing it, while dirty force-removals leave completion opt-in. The branch ref is retained either way. An idle embedded session can be closed as part of linked-worktree removal by explicitly enabling `Force remove`; a session with an active turn still blocks removal. `/delete` and `/forget` are aliases.
