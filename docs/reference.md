...
- `p` pin toggle
- `I` review a primary checkout that differs from its saved home branch for any selected member of the repository family
- `q` quit
...
- Consecutive identical command and file-change blocks collapse into one row with an `×N` count; `Alt+L` opens a transcript-detail picker (`1`-`4` choose directly, `Enter` applies, `Esc` cancels). The default Steps level folds each run of tool traffic under the engineer's prose into intent-level activity lines (edited files, test and check results, searches, reads, other commands, scratch probes), keeps failures and the live in-flight call on their own lines, and leaves raw commands out; Tool calls shows each call with output hidden, Previews adds short output previews, and Full output reveals everything, including every collapsed occurrence
...
- `/integrity` (`I`): Review a primary checkout that differs from its saved home branch, hand it to a fresh engineer, dismiss the exact warning, keep the current branch as home, or apply a conservative linked-worktree repair.
...
- `/codex-gc`: Open the audit-first Codex storage cleanup form. Project rows show REMOVE, KEEP, and emphasized MiB/GiB sizes; sorting defaults to largest first, with age and name available in the Sort dropdown. Storage breakdown shows whole-home logical totals and read-only retained files grouped by project or storage entry.
...
- `/handoff [note]`: Choose the target embedded agent, then choose that agent's model and reasoning effort before LCR saves a bounded, host-generated continuation brief from cached session state and starts the fresh session with a prompt that points to it. The source session stays unchanged while either picker is open. This does not ask the source session or another model to summarize itself, so it remains usable when the current provider stream is unavailable. The private `0600` Markdown brief is stored below the app data directory under `embedded-sessions/handoffs/`; it omits private reasoning, may be incomplete, and tells the fresh session to inspect the repository and running processes before continuing.
- `/lcagent-handoff [note]`: Create the same provider-independent continuation brief, then replace the current embedded session with a fresh LCAgent session. Use this when Codex or another provider cannot safely complete another turn, including provider-policy false positives. LCAgent uses its configured provider, model, reasoning, permissions, and workspace contract; the source session remains resumable from its own artifact store.
- `/image-review [on|off]`: Report or explicitly change external image recovery for this idle Codex session. Enabling reconnects the exact thread and exposes a text-returning API reviewer using `gpt-5.6-luna` / `high`; the banner shows when it is enabled. New sessions default to off, and no automatic fallback is installed. Requires OpenAI API credentials and uses separate API billing. See [image review recovery](image_review_recovery.md).
...
- `/fast [on|off|status]` (Codex): Inspect or change the shared fast-mode default. Bare `/fast` shows status; `/fast on` explicitly enables higher usage for at most two hours and `/fast off` selects standard speed. The shared countdown survives session switches and restarts; re-enable explicitly after expiry. The red **FAST** indicator appears by the model/reasoning in the sidebar and in the engineer header. A running fast turn keeps its warning after disabling; its next turn uses standard speed. See [Codex fast mode](codex_fast_mode.md) for persistence and synchronization details.
...
- `/codex-gc`: Leave the embedded pane visible behind the global cleanup form. Choose Orphaned worktrees (default) or Stale sessions in the Clean up dropdown. Stale sessions offers an inactivity threshold of 7, 14, 30, or 90 days and always keeps the newest root session per folder plus protected trees. Tab moves focus, arrows navigate, Space selects project rows, and Enter activates the focused control. Mouse clicks also work. Storage breakdown is a separate read-only view; Back/Esc preserves the cleanup selection. Review cleanup opens the explicit permanent-deletion confirmation.
...
- Completed agent tasks remain in the project list until explicitly removed. `/remove` asks for confirmation, then makes the selected item go away using the safest matching action: it moves an agent task to Trash for seven days before automatic deletion, opens scratch-task archive/delete actions, cleans up linked worktrees, removes missing folders from the dashboard, or hides a regular project's exact path from the list. A linked worktree with an originating TODO offers to mark that TODO done before removal even when the branch has not been merged; clean checkouts default to completing it, while dirty force-removals leave completion opt-in. The branch ref is retained either way. An idle embedded session can be closed as part of linked-worktree or agent-task removal; a session with an active turn still blocks removal. `/delete` and `/forget` are aliases.
...
- `/clean` audits linked worktrees across the project list. A worktree is eligible only when it is present, unpinned, merged into its recorded parent branch, clean, conflict-free, on a known completed latest turn whose latest assessment is `done`, and its latest real activity is more than 24 hours old. Active engineer turns, approvals/input requests, externally owned live sessions, runtimes, pending worktree launches, conflict resolvers, and in-flight Git actions exclude a candidate. Every eligible item starts checked. Space toggles it and Enter begins removal. Immediately before each removal LCR refreshes Git-derived state, repeats the durable eligibility checks, and checks live runtime/session state again; anything that changed is skipped and the batch continues. An idle LCR-managed engineer session is closed first, an originating TODO is marked done through the merged-worktree finalization path, and the checkout is removed without force. Git branches and AI conversation history are preserved. Escape cancels the audit or requests cancellation of removal and closes the dialog immediately so the rest of the TUI remains usable. Reopen `/clean` to view a settling operation or its retained report; queued items are skipped and completed removals remain applied. The current checkout stays reserved until its operation settles. Process-use checks have a 10-second limit; open-file blockers are skipped with process names/PIDs and an instruction to close those apps before retrying. The final report separates removed, skipped, and failed worktrees, including any partial removal errors.
- `/codex-gc` defaults to orphaned Codex sessions whose recorded worktree is absent and exactly matches a retained LCR deleted-worktree record. It excludes pinned or loaded trees, activity newer than 7 days, worktrees missing for less than 7 days, external-volume paths, and uncertain path, lineage, descendant, or rollout evidence. Stale cleanup covers existing folders using the selected 7/14/30/90-day threshold, while preserving the newest root session per folder and protected trees. Each row summarizes REMOVE / KEEP counts, including descendants, and recoverable rollout size. Space toggles rows; the select-all control or `A` while the table is focused toggles all. Review cleanup opens a separate irreversible-deletion warning with exact counts, defaulting to Back; only explicit activation of Delete N sessions permanently proceeds. LCR repeats the safety audit with the same threshold, rejects changed preview revisions, deletes selected roots through Codex app-server `thread/delete`, and verifies selected rows and rollout files are gone before reporting reclaimed logical bytes. During deletion, `B` hides progress and `/codex-gc` reopens it. Esc aborts the active request and skips queued groups; completed deletion stays permanent and is still verified. Periodic background audits are orphaned-only, read-only, and never delete sessions.
...
- LCR stores an evidence-backed home branch for each repository family's primary checkout, preferring `origin/HEAD` and keeping that policy separate from linked-worktree merge targets. It warns when the primary checkout is found on another branch. Press `I` or run `/integrity` for details, a fresh investigation-first engineer handoff, exact-state dismissal, explicit home-branch update, or conservative repair. The default is warn-only; see [`repository_root_integrity.md`](repository_root_integrity.md).
...
- `/handoff [note]` is the fresh-thread or cross-provider recovery path when the current stream or session cannot be trusted to answer. LCR first collects the target agent/model/reasoning choices, writes the continuation brief itself from cached structured entries, and opens the selected fresh session only after that write succeeds; if provider launch subsequently fails, the error retains the brief path for manual recovery.
- `/lcagent-handoff [note]` is the direct LCAgent recovery shortcut. It does not submit a compaction request to the blocked source provider; LCR writes the brief first and then starts LCAgent with only the brief's private local path in the launch prompt.
