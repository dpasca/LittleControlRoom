...
- `/codex-gc`: Leave the embedded pane visible behind the global Codex session-storage cleanup dialog. The header offers `1` Orphaned worktrees (green, default) and `2` Stale sessions (amber; inactive at least seven days in existing folders, with newest root session and protected trees retained). Tab switches between Cleanup candidates and blue, read-only Other storage; its hint names the destination. Stale rows summarize REMOVE / KEEP counts per folder. Selection totals sit beside the separate deletion-review action. `C` and `V` remain compatibility shortcuts.
...
- `/codex-gc` audits Codex storage for root threads whose recorded worktree is still absent and exactly matches a retained LCR deleted-worktree record. It excludes pinned or loaded trees, activity newer than 7 days, worktrees missing for less than 7 days, external-volume paths, and any uncertain path, lineage, descendant, or rollout evidence. Eligible roots are grouped by worktree with age, branch/commit metadata, descendant counts, reason, and recoverable rollout size. Space toggles individual groups and `A` selects or deselects all; Enter opens a separate irreversible-deletion warning, and only `D` proceeds. LCR repeats the safety audit, deletes selected roots through Codex app-server `thread/delete`, and verifies the selected root/descendant rows and rollout files are gone before reporting reclaimed logical bytes. During deletion, `B` returns to the rest of the TUI while the job continues and `/codex-gc` reopens its progress or report. Esc aborts the active app-server request and skips queued groups; already completed deletion remains permanent and is still verified before the final report. The periodic background audit is read-only and never deletes sessions.
