...
- `/codex-gc` audits Codex storage for root threads whose recorded worktree is still absent and exactly matches a retained LCR deleted-worktree record. It excludes pinned or loaded trees, activity newer than 30 days, worktrees missing for less than 7 days, external-volume paths, and any uncertain path, lineage, descendant, or rollout evidence. Eligible roots are grouped by worktree with age, branch/commit metadata, descendant counts, reason, and recoverable rollout size. Space explicitly selects groups; Enter opens a separate irreversible-deletion warning; only `D` proceeds. LCR repeats the safety audit, deletes selected roots through Codex app-server `thread/delete`, and verifies the selected root/descendant rows and rollout files are gone before reporting reclaimed logical bytes. The periodic background audit is read-only and never deletes sessions.
