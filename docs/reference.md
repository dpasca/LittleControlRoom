...
- Embedded Claude Code runs through Claude Code's `claude -p` stream flow. Prompt/response turns, session resume, `/model`, context-usage reporting, and native `/compact [instructions]` are wired. LCR only reports successful Claude compaction after receiving Claude's structured compaction boundary; a short conversation can therefore return a clear no-op result.
