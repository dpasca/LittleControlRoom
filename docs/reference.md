...
- `/remove` asks for confirmation, then makes the selected item go away using the safest matching action: it opens scratch-task archive/delete actions, cleans up linked worktrees, removes missing folders from the dashboard, or hides a regular project's exact path from the list. An idle embedded session can be closed as part of linked-worktree removal by explicitly enabling `Force remove`; a session with an active turn still blocks removal. `/delete` and `/forget` are aliases.
