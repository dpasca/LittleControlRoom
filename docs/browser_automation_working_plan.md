...
- Background Chromium sessions load an LCR-owned extension that mutes all tabs at startup and follows the shared managed-browser visibility state. Audio is permitted only after a successful native reveal and is muted again on the next hide request; newly created tabs and popups inherit the policy. The extension uses native tab muting, preserves user-applied mutes, and returns to muted if its session-private loopback connection closes. Visibility changes reach it on a 250ms reconciliation interval, and ten-second heartbeats keep its service worker active. Both MCP launch paths and the direct browser worker share this setup. `Always show` and true headless launches retain Playwright's defaults.
- Automatic audio restoration requires an extension-capable Chromium executable (including Playwright Chromium or Chrome for Testing). Branded Chrome/Edge and unknown custom executables instead receive `--mute-audio`, remain silent during handoffs, and emit an explicit startup notice. `lcroom browser status` reports `audio_mode` (`visibility` or `muted`) and the `audio_allowed` reveal intent. Changes apply to newly launched browser processes; an already-running wrapper must reconnect after updating LCR. The macOS startup focus flash remains a separate launch/activation issue.
- The macOS background browser monitor first asks the PID-specific AppKit application object to hide, including during Chromium's launch transition, then uses launch-gated Accessibility hiding as a fallback. It keeps re-hiding the process while LCR marks it hidden, which reduces the interval in which a newly launched Chromium window can take focus.
- Managed browser reveal records a cross-session foreground marker before raising the macOS window. Background monitors protect the exact foreground browser PID when it is known and use application identity only as a conservative fallback, so a revealed handoff stays visible without exempting every sibling Chromium process from background hiding.
- On macOS, `ctrl+o` reveals or refocuses the existing `Only when needed` browser because that launch mode is background-headed rather than truly headless. A legacy session that actually launched with `--headless` now fails before changing reveal state and asks for a reconnect, since that process has no native window to raise.
- The next successfully submitted user message re-arms background hiding and clears that session's foreground marker under the shared visibility lock. This applies to managed Codex, OpenCode, Claude Code, and LCAgent sessions, while `Always show` remains visible.
...
- Playwright/runtime MCP readiness waits now use startup-specific deadlines and cache both servers' startup notifications. The actual turn RPC always receives a fresh deadline, so a slow runtime registration cannot falsely reject an accepted message, restore it into the composer, or leave a completed browser handoff pending.
...
- Managed-agent guidance now treats Playwright's `browser_resize` as per-tab viewport emulation, not native window resizing. Responsive checks belong in disposable tabs, and human handoffs use a fresh, unresized tab in the same browser context so the revealed window remains manually resizable without reloading or mutating the handoff page after it is ready.
- LCAgent's headed fallback worker now leaves the Playwright viewport unset so it follows the native browser window. Its headless mode retains a deterministic `1280x900` viewport.
...
- Browser waits now raise the centered attention dialog even while the affected embedded session is visible, unless another provider input dialog already owns the foreground. The dialog shows the request's absolute time and age, including the original turn time when Codex reconstructs a handoff after resume, so an old project name cannot make a fresh request look stale and an old request cannot look newly emitted. Dismissing it acknowledges that specific handoff while leaving the Browser sidebar and `ctrl+o` available; a changed instruction or failed reveal can surface it again.
...
- Managed Codex login flows now go through an LCR-owned interactive browser lease:
  ...
  - an interactive reservation expires after ten minutes without newer session/browser activity or a lease renewal, so an abandoned handoff cannot block later work indefinitely
  - only recent competing flows block a later session; their browser-attention dialog offers an explicit takeover, while the previous flow remains waiting in its own session instead of being closed or discarded
...
- After a managed browser handoff, the next accepted user message returns `Only when needed` browsers to that re-hidden state without blocking submission on macOS window control.
...
- Browser-attention coverage now verifies the exact structured tool identity, required instruction, stale or mismatched managed state rejection, failed tool results, idle and resume persistence with the original request time, successful-response clearing, inactivity protection, stale-lease recovery, popup acknowledgement/retry/takeover behavior, and OpenCode parity.
...
- macOS window-control coverage now verifies early AppKit background hiding with a launch-gated Accessibility fallback, exact-PID foreground protection, submission-triggered re-hiding, background-headed `ctrl+o` reveal, explicit rejection of truly headless reveal attempts, PID-targeted activation postconditions, bounded verified retries, retained `(-600)` diagnostics, and termination of hung commands without requiring a live UI.
- Managed-state retention coverage verifies live owner/PID and Chromium singleton protection, shared-profile safety, age expiry, oldest-first ceiling eviction, and periodic audit logging.
...
- Never delete a managed session or profile solely to satisfy retention or a disk ceiling while its owner, MCP, browser process, shared-profile reference, or Chromium singleton indicates live use.
